...
/** Firestore del emulador por Admin SDK (para sembrar lo que la API no deja). */
function emuDb() {
...
/** Último oobCode de PASSWORD_RESET para un correo (emulador de Auth). */
async function lastResetCode(request, email) {
  ...
  return [...oobCodes].reverse().find((c) => c.email === email && c.requestType === "PASSWORD_RESET")?.oobCode ?? null;
}
