...
/** Marca el correo como verificado (equivale a aplicar el oobCode VERIFY_EMAIL). */
async function verifyEmailInEmulator(request, idToken, email) {
  const user = await qaAuthAdmin().getUserByEmail(email);
  await qaAuthAdmin().updateUser(user.uid, { emailVerified: true });
}
...
test.describe.serial("QA · Editar cliente — correo hasta verificar, teléfono único y espejo en users", () => {
  let adminToken;
  ...
  test("2) cuenta SIN verificar: cambiar el correo actualiza Auth, users y clients, y reenvía la activación", { tag: ["@api"] }, async ({ request }) => {
    ...
    const auth = await request.post(`https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${apiKeyPublica()}`, {
